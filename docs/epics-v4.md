# yaga.ai — Epic Draft v4

Status: draft v4
Owner: James
Review basis: v3 + third review loop from Naomi, Amos, Alex
Scope: epics only, no stories/tasks yet
Intent: define a v1 plan that is operationally provable, architecturally coherent, and guarded against accidental generalization
...
- **EPIC-05 artifact contracts** are prerequisites for artifact-aware observability in EPIC-06
...
Supported flow definitions only:
...
- every generic-flow primitive in v1 must be exercised by at least one of the two supported loops, or it is out of scope
